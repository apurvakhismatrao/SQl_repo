...
select FullName, replace(FullName, "John", "JACK") AS NEW 
from employee;

select * from employee;
SELECT FullName, reverse(FullName) as REVERSED
from employee;

SELECT FullName, length(FullName) as Char_Length
from employee;

select substring('Good Moeging',3,5) as extractstring,
 length(substring('Good Moeging',3,5)) as clk;
 
 select substring(FullName,1,3) as clipped from employee;
